...
export default function TopSpecialties({
  ...
  const totalAppointments = topSpecialties.reduce(
    (sum, specialty) => sum + specialty.appointments,
    0,
  );
  return (
      ...
      <CardContent>
        ...
        <div className="space-y-6">
          {topSpecialties.map((specialty) => {
            ...
            // Porcentagem de ocupação da especialidade baseando-se no total de agendamentos
            const progressValue =
              (specialty.appointments / totalAppointments) * 100;
